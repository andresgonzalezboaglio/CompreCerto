                ...
                try:
                    ...
                    if 'itemListElement' in data:
                        ...
                        for item in productos:
                            ...
                            # Verificar que el nombre exista antes de extraer la cantidad y unidad de medida
                            if nombre:
...
def guardar_precios_bistek(searchTerms):
    resumen_guardados = {}
        ...
        for producto in productos:
            ...
            producto_obj, created = Producto.objects.update_or_create(
                ...
                defaults={
                    ...
                    'fecha_aumento': None  # Cambiado a fecha_aumento
                }
            )
            ...
            print(f"BISTECK: Producto guardado/actualizado: {nombre} - Precio: {precio}")

        resumen_guardados[searchTerm] = productos_guardados

    print("Resumen final de productos guardados para Bistek:")
    for term, count in resumen_guardados.items():
        print(f"{count} productos guardados/actualizados para el término '{term}'.")
...
def guardar_productos_stock_center(searchTerms):
    supermercado, _ = Supermercado.objects.get_or_create(
        nombre="Stock Center",
    ...
    for searchTerm in searchTerms:
        ...
        for producto in productos_extraidos:
            ...
            productos_guardados += 1
            print(f"STOCK CENTER: Producto guardado/actualizado: {nombre} - Precio: {precio}")

        resumen_guardados[searchTerm] = productos_guardados

    print("Resumen final de productos guardados para Stock Center:")
    for term, count in resumen_guardados.items():
        print(f"{count} productos guardados/actualizados para el término '{term}'.")


# Uso de las funciones desde la consola de Python
def ejecutar_scraping(searchTerms):
    print("Guardando productos de Bistek...")
    guardar_precios_bistek(searchTerms)

    print("\nGuardando productos de Stock Center...")
    guardar_productos_stock_center(searchTerms)


# Ejemplo de cómo llamarlo
searchTerms = ["canela"]
ejecutar_scraping(searchTerms)
