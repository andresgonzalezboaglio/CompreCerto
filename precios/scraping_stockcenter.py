...
def obtener_ofertas_stock_center(searchTerm):
    ...
    while True:
        ...
        if response.status_code == 200:
            ...
            if pagina_actual >= paginator.get('total_pages', 1):
                break
            pagina_actual += 1
        else:
            break
...
def guardar_productos_stock_center():
    ...
    for searchTerm in searchTerms:
        ...
        for producto in productos:
            nombre = producto['descripcion'].strip()
        ...
        print(f"STOCK CENTER: Se han guardado productos para el término '{searchTerm}'.")
